...
static int parse_command_line(DaemonConfig *c, int argc, char *argv[]) {
    ...
    assert(c);
    while ((o = getopt_long(argc, argv, "hDkVif:rcs", long_options, NULL)) >= 0) {
        switch(o) {
            case 's':
                c->use_syslog = 1;
                break;
...
static const char* pid_file_proc(void) {
		...
		static char *fn = NULL;
		free(fn);
		asprintf(&fn, "%s%s-%s.pid", AVAHI_DAEMON_RUNTIME_DIR, daemon_pid_file_ident ? daemon_pid_file_ident : "unknown", instance);
	#else
		static char fn[PATH_MAX];
		snprintf(fn, sizeof(fn), "%s%s-%s.pid", AVAHI_DAEMON_RUNTIME_DIR, daemon_pid_file_ident ? daemon_pid_file_ident : "unknown", instance);
	#endif
	return fn;
}
